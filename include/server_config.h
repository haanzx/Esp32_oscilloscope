    ...
    // ----- WiFi network -----
        ...
        void WiFi_start (threadSafeFS::FS& fileSystem) {
            ...
            if (STA_SSID [0] != '\0') {
                ...
                if (!STA_DHCP) {
                    ...
                    IPAddress sta_ip, sta_gw, sta_sn, sta_dns1, sta_dns2;
                    sta_ip.fromString(STA_IP);
                    sta_gw.fromString(STA_GATEWAY);
                    sta_sn.fromString(STA_SUBNET_MASK);
                    sta_dns1.fromString(STA_DNS_1);
                    sta_dns2.fromString(STA_DNS_2);
                    WiFi.config(sta_ip, sta_gw, sta_sn, sta_dns1, sta_dns2);
            ...
            if (AP_SSID [0] != '\0') {
                ...
                if (AP_IP [0] != '\0') {
                    ...
                    ap_sn.fromString(AP_SUBNET_MASK);
                    WiFi.softAPConfig(ap_ip, ap_ip, ap_sn);
                }
        ...
        void WiFi_start () {
            ...
            if constexpr (default_sta_ssid [0] != '\0') {
                ...
                // Deklarasikan objek IPAddress terlebih dahulu
                IPAddress sta_ip, sta_gw, sta_sn, sta_dns1, sta_dns2;

                // Konversi string makro ke objek IPAddress
                sta_ip.fromString(DEFAULT_STA_IP);
                sta_gw.fromString(DEFAULT_STA_GATEWAY);
                sta_sn.fromString(DEFAULT_STA_SUBNET_MASK);
                sta_dns1.fromString(DEFAULT_STA_DNS_1);
                sta_dns2.fromString(DEFAULT_STA_DNS_2);

// Masukkan objek yang sudah siap ke WiFi.config
WiFi.config(sta_ip, sta_gw, sta_sn, sta_dns1, sta_dns2);
            ...
            if (default_ap_ssid [0] != '\0') {
                ...
                if (WiFi.softAP (DEFAULT_AP_SSID, DEFAULT_AP_PASSWORD)) { 
                    ...
                    // Deklarasikan objek IPAddress
                    IPAddress def_ap_ip, def_ap_sn;

                    // Konversi string makro
                    def_ap_ip.fromString(DEFAULT_AP_IP);
                    def_ap_sn.fromString(DEFAULT_AP_SUBNET_MASK);

                    // Masukkan ke WiFi.softAPConfig
                    WiFi.softAPConfig(def_ap_ip, def_ap_ip, def_ap_sn);
